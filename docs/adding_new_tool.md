...
If you're creating a new version of an existing tool that should replace it, you can use the **supersession mechanism**.
This ensures that when the new tool is available, the old tool is automatically disabled and removed from the registry.
...
1. **Automatic Removal**:
   - When the registry initializes, it automatically removes the superseded tool from `_enabled_tools`
   - The superseded tool is also removed from `_preapproved_tool_names`
   - This happens in the `_remove_superseded_tools()` method
...
- It also supersedes `RunCommand`, so `RunCommand` won't be available when `ShellCommand` is enabled
