...
def mock_prompt_security_fixture():
    ...
    with patch(
        "duo_workflow_service.agent_platform.experimental.components.one_off.nodes.tool_node_with_error_correction.apply_security_scanning"
    ) as mock_security:
...
def mock_logger_fixture():
    ...
    with patch(
        "duo_workflow_service.agent_platform.experimental.components.one_off.nodes.tool_node_with_error_correction.structlog"
    ) as mock_structlog:
        mock_logger = Mock()
...
def mock_tool_monitoring_fixture():
    ...
    with patch(
        "duo_workflow_service.agent_platform.experimental.components.one_off.nodes.tool_node_with_error_correction.duo_workflow_metrics"
    ) as mock_metrics:
