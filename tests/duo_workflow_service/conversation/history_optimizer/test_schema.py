...
class TestCompactionResult:
    ...
    @pytest.mark.parametrize(
        "was_modified,summary,error,expected",
        [
            (True, AIMessage(content="s"), None, True),
            (False, AIMessage(content="s"), None, False),
            (True, None, None, False),
            (True, AIMessage(content="s"), RuntimeError("e"), False),
            (False, None, RuntimeError("e"), False),
        ],
    )
    def test_succeeded(self, was_modified, summary, error, expected):
        """``succeeded`` is True only when compacted, has a summary, and no error."""
        result = CompactionResult(
            was_modified=was_modified, summary=summary, error=error
        )
...
class TestCompactionConfig:
    ...
    def test_rejects_non_positive_max_recent_messages(self):
        with pytest.raises(
    ...
    def test_rejects_invalid_trim_threshold(self):
        # Must be in (0, 1].
