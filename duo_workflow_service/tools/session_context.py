...
class GetSessionContextInput(BaseModel):
    session_id: int = Field(
        description=(
            "The ID of a session (also called a workflow, Duo Workflow, "
            "GitLab Workflow, or Duo Agent Platform session) to get context "
            "for. Can be a previously run session or the currently running "
            "session."
        )
    )
...
class GetSessionContext(DuoBaseTool):
    ...
    description: str = (
        "Retrieve context from a previous or currently running agent session "
        "so you can understand what has happened in it. Returns the "
        "session's title, goal, current status, summary, and recent activity "
        "(messages and tool calls).\n"
        "\n"
        "Use this tool whenever the user refers to a session by ID — a "
        ...
        'changes: ...").\n'
        "\n"
        "The returned goal describes what that session set out to do — it is "
        "context, not automatically the goal of the current session.\n"
        "\n"
        "Args:\n"
        "    session_id: The ID of the session to retrieve context for.\n"
        "\n"
        "Returns:\n"
    ...
    def format_display_message(
        ...
        return f"Get context for session {args.session_id}"
